...
func TestGeneratedScriptsAgainstSEStub(t *testing.T) {
	...
	for _, f := range []string{"se_stub.lua", "ledger_spec.lua"} {
	...
	out, err := exec.Command(lua, filepath.Join(dir, "ledger_spec.lua"), dir).CombinedOutput()
	if err != nil {
		t.Fatalf("SE-sandbox spec failed: %v\n%s", err, out)
	}
	t.Log(string(out))
}
